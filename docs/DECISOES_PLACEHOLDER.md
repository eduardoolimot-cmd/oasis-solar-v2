...
## Geração prevista divergente do cadastro por SKID — correção (2026-09-23)

- Sintoma: UFV Cerado Pedra III mostrava em agosto "Geração prevista (PVsyst)" 16.275 kWh, mas o SKID 01 tem 10.079 kWh cadastrados. O mesmo em Cerado Pedra II, Efizi e Nacional I: a previsão da usina tinha o mesmo valor nos 12 meses.
- Causa (duas partes): (1) a planilha Dados_de_Geração_Mensal_2.xlsx (Planilha1) traz, para essas 4 usinas, um valor mensal constante (ex.: 16,865 MWh em todos os meses para Cerado Pedra III), gravado na previsão da usina pela importação de 2026-09-20; (2) ao salvar os dados mensais por SKID, o recálculo da previsão da usina descartava SKIDs sem potência FV cadastrada — os 4 SKIDs dessas usinas não têm potência — e mantinha o valor antigo.
- Regra corrigida (lib/previsaoUsinaSkid.ts, usada pela rota e pelo script): geração prevista da usina = soma do E_Grid de todos os SKIDs ativos, **sem depender de potência** (soma de energia); irradiação/PR = valores do SKID quando a usina tem um só, ponderados por potência quando tem vários (exige potência em todos; senão preserva o anterior). Mês em que algum SKID não tem valor: preserva o anterior.
- Dados corrigidos com `prisma/recalcular_previsao_usina_skid.ts --usina ... --aplicar` só nas 4 usinas (novas versões; anteriores preservadas; backup antes). Agosto Cerado Pedra III: 10.079 kWh × 0,965 (degradação ano 4) = 9.726 kWh.
- **Não alterado, decisão do cliente:** (a) P50/P90 dessas 4 usinas continuam constantes, pois vêm da mesma Planilha1 constante (P50 = Planilha1 × fator de Premissas) — ex.: Cerado Pedra III P50 17.032 kWh/mês, soma anual ≈ 204 MWh contra ≈ 146 MWh do E_Grid anual por SKID; (b) PR previsto da usina difere do PR ponderado dos SKIDs em Camaçari, Malhada, Paratinga e Pedro Canário (Pedro Canário: 90–99% na usina × 77–85% nos SKIDs), valores vindos de importações diferentes; (c) Cerado Pedra I: dezembro 12.629 kWh na usina × 12.585 no SKID (igual a novembro no SKID — possível erro de digitação); (d) arredondamentos de irradiação (ex.: 207,7 × 207) em Bom Futuro e Cerado Pedra I.
## Estoque — cadastro completo do material e ficha do item (2026-09-23)

- "+ Novo material" (aba Estoque) abre um formulário com todos os campos obrigatórios: nome, categoria (lista com as categorias já usadas, aceitando nova), número da etiqueta, unidade (lista com as já usadas + un, pç, m, kg, L, cx, rolo, par, jogo), quantidade e foto (JPG/PNG/WebP até 10 MB, com prévia). Custo unitário é opcional: sem ele, a entrada fica sem custo e o custo médio mostra "Sem custo apurado" (nunca presumido). Se depois houver entrada com custo, o custo médio passa a ser o dessa entrada (as unidades sem custo não entram na média).
- Nova rota `POST /estoque/:usinaId/itens` (multipart) cria item, foto e a quantidade inicial como ENTRADA na usina numa só transação ("Quantidade inicial no cadastro do item"); exige permissões estoque.criar e estoque.movimentar. Validações: todos os campos, assinatura real da imagem, quantidade ≥ 0; foto recusada é apagada do disco.
- Número da etiqueta: novo campo `ItemCatalogo.numeroEtiqueta` (opcional no banco — itens antigos não têm; editável em "Editar"). Não pode repetir entre itens: conferido na API (409 com o nome do item que já usa a etiqueta). Sem `@unique` no banco de propósito: com ele, o `prisma db push` pede `--accept-data-loss`, que o guia de deploy manda nunca confirmar.
- Clicar na linha do item abre a ficha: foto, nome, categoria, etiqueta, unidade, saldo, custo médio, total de entradas e de saídas e o histórico completo de movimentações do item na usina (a lista geral continua limitada às últimas 200), com botões "Registrar entrada/saída". Os botões da linha (imagem, Entrada, Saída, Ajustar, Editar, Excluir) não abrem a ficha.
- Testado pela interface e pela API (cadastro completo, etiqueta repetida, sem foto, sem categoria, quantidade inválida, arquivo que não é imagem, saída pela ficha). Item, movimentações, registros de auditoria e foto de teste apagados.